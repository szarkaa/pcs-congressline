...
public class OrderedOptionalServiceResource {
    ...
    public ResponseEntity<OrderedOptionalServiceDTO> create(@Valid @RequestBody OrderedOptionalServiceVM viewModel) throws URISyntaxException {
        log.debug("REST request to save ordered optional service : {}", viewModel);
        if (viewModel.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil
                .createFailureAlert(ENTITY_NAME, "idexists", "A new ordered optional service cannot already have an ID"))
                .body(null);
        }
        OrderedOptionalService result = service.save(viewModel);
        return ResponseEntity.created(new URI("/api/ordered-optional-services/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, result.getId().toString()))
            .body(new OrderedOptionalServiceDTO(result));
    }

    @SuppressWarnings("MissingJavadocMethod")
    @PutMapping("/ordered-optional-services")
    public ResponseEntity<OrderedOptionalServiceDTO> update(@Valid @RequestBody OrderedOptionalServiceVM viewModel) throws URISyntaxException {
        log.debug("REST request to update ordered optional service : {}", viewModel);
        if (viewModel.getId() == null) {
    ...
    public List<OrderedOptionalServiceDTO> getAllDTOsByRegistrationId(@PathVariable Long id) {
        ...
        return service.findAllByRegistrationId(id).stream().map(oos -> {
            ...
            dto.setPriceWithDiscount(discountService.getPriceWithDiscount(oos.getPayingGroupItem(), oos.getChargeableItemPrice(), priceService.getScale(oos)));
            return dto;
    ...
    public ResponseEntity<OrderedOptionalServiceDTO> getById(@PathVariable Long id) {
        log.debug("REST request to get ordered optional service : {}", id);
        return service.findById(id)
            .map(result -> new ResponseEntity<>(new OrderedOptionalServiceDTO(result), HttpStatus.OK))
    ...
    public ResponseEntity<Void> delete(@PathVariable Long id) {
        log.debug("REST request to delete ordered optional service : {}", id);
        try {
            service.delete(id);
